class constains {
  final List listdanhsach = [
    {
      'name': 'Thời Trang Nam',
      '0': [
        'Áo thun',
        'Áo sơ mi',
        'Áo khoác & Áo vest',
        'Áo nỉ/Áo len',
        'Đồ bộ/Đồ mặc nhà',
        'Đồ Đôi',
        'Quần',
        'Balo/Túi/Ví',
        'Mắt Kính',
        'Phụ Kiện nam',
        'Đồ Trung Niên',
        'Trang Sức nam',
        'Thắt lưng',
        'Đồ lót'
      ].toList()
    },
    {'name': 'Thời Trang Nữ'},
    {'name': 'Điện Thoại& Phụ Kiện'},
    {'name': 'Mẹ & Bé'},
    {'name': 'Thiết Bị Điện Tử'},
    {'name': 'Nhà Cửa & Đời Sống'},
    {'name': 'Máy tính & Laptop'},
    {'name': 'Sức Khỏe & Sắc Đẹp'},
    {'name': 'Máy ảnh - Máy quay phim'},
    {'name': 'Giầy Dép Nữ'},
    {'name': 'Đồng Hồ'},
    {'name': 'Túi Ví'},
    {'name': 'Giầy Dép Nam'},
    {'name': 'Phụ Kiện Thời Trang'},
    {'name': 'Thiết Bị Điện Gia Dụng'},
  ];
  final List select = [
    {
      '0': [
        {
          'name': 'Áo thun',
          '00': [
            {'name': 'Áo ngắn tay có cổ'},
            {'name': 'Áo ngắn thay không cổ'},
            {'name': 'Áo thun dài tay'},
            {'name': 'Áo nỉ'},
            {'name': 'Áo len'},
            {'name': 'Khác'}
          ]
        },
        {
          'name': 'Áo sơ mi',
          '00': [
            {'name': 'Khác'},
            {'name': 'Áo kiểu'},
            {'name': 'Cổ tàu'},
            {'name': 'Dài tay'},
            {'name': 'Ngắn tay'}
          ]
        },
        {
          'name': 'Áo khoác & Áo vest',
          '00': [
            {'name': 'Áo khoác nhẹ'},
            {'name': 'Áo khoác dù'},
            {'name': 'Áo Vest'},
            {'name': 'Áo khoác dạ'},
            {'name': 'Áo khoác phao'},
            {'name': 'khác'},
            {'name': 'Áo blazer'}
          ]
        },
        {'name': 'Áo nỉ/Áo len'},
        {'name': 'Đồ bộ/Đồ mặc nhà'},
        {'name': 'Đồ Đôi'},
        {'name': 'Quần'},
        {'name': 'Balo/Túi/Ví'},
        {'name': 'Mắt Kính'},
        {'name': 'Phụ Kiện nam'},
        {'name': 'Đồ Trung Niên'},
        {'name': 'Trang Sức nam'},
        {'name': 'Thắt lưng'},
        {'name': 'Đồ lót'}
      ].toList()
    },
    {
      '1': [
        {'name': 'Áo'},
        {'name': 'Đầm'},
        {'name': 'Chân váy'},
        {'name': 'Quần'},
        {'name': 'Set trang phuck & jumpsuit'},
        {'name': 'Đồ Đôi'},
        {'name': 'đồ lót,Đồ ngủ & Đồ mặc nhà'},
        {'name': 'Đồ bơi'},
        {'name': 'Trang phục thể thao'},
        {'name': 'Phụ kiện may mặc'},
        {'name': 'Thời trang trung niên'},
        {'name': 'Áo khoác & Áo vest'},
        {'name': 'Trang Phục Đông'},
        {'name': 'Thời trang bầu và sau sinh'}
      ].toList()
    },
    {
      '2': [
        {'name': 'Điện thoại'},
        {'name': 'Máy tính bảng'},
        {'name': 'Vỏ,Bao & Ốp lưng'},
        {'name': 'Miếng dán màn hình'},
        {'name': 'Pin sạc dự phòng'},
        {'name': 'Pin,Cáp & Bộ sạc'},
        {'name': 'Giá đỡ & kẹp'},
        {'name': 'Gậy chụp hình'},
        {'name': 'Sim'}
      ].toList()
    },
    {
      '3': [
        {'name': 'Đồ dùng cho bé'},
        {'name': 'Tã & Bỉm'},
        {'name': 'Ghế,Nôi,Cũi,Xe đẩy & Địu'},
        {'name': 'Vitamin & đồ dùng cho mẹ'},
        {'name': 'Sữa công thức 0-24 tháng tuổi'},
        {'name': 'Thực Phẩm bổ sung cho trẻ dưới 6 tháng tuổi'},
        {'name': 'Sữa trên 24 tháng tuổi'},
        {'name': 'Thực phẩm cho bé từ 6 tháng tuổi'},
        {'name': 'Vitamin,thực phẩm bổ sung và đồ dùng cho mẹ'}
      ].toList()
    },
    {
      '4': [
        {'name': 'Thiết bị âm thanh'},
        {'name': 'Tai nghe'},
        {'name': 'Thiết bị đeo thông minh'},
        {'name': 'Android Tivi box'},
        {'name': 'Tivi'},
        {'name': 'Phụ kiện tivi'},
        {'name': 'Tai Nghe'},
        {'name': 'Phụ kiên & Thiết bị game'}
      ].toList()
    },
    {
      '5': [
        {'name': 'Dụng cụ & Thiết bị tiện ích'},
        {'name': 'Đồ dùng nhà bếp & Phòng ăn'},
        {'name': 'Chăn,Ga,Gối & Nệm'},
        {'name': 'Tủ đựng & Hộp lưu trữ'},
        {'name': 'Trang trí nhà cửa'},
        {'name': 'Đồ dùng phòng tắm'},
        {'name': 'Đồ nội thất'},
        {'name': 'Đèn'},
        {'name': 'Ngoài trời & Sân vườn'},
        {'name': 'Dụng cụ cầm tay'}
      ].toList()
    },
    {
      '6': [
        {'name': 'Laptop'},
        {'name': 'máy Tính Bàn'},
        {'name': 'Linh Kiện Máy Tính'},
        {'name': 'Chuột,Bàn Phím'},
        {'name': 'Thiết Bị Mạng'},
        {'name': 'USB & Ổ Cứng'},
        {'name': 'Máy In,Máy Scan & Máy Chiếu'},
        {'name': 'Phần Mềm'},
        {'name': 'Phụ Kiện Máy Tính khác'}
      ].toList()
    },
    {
      '7': [
        {'name': 'Chăm sóc da'},
        {'name': 'Son & Chăm sóc môi'},
        {'name': 'Trang điểm da'},
        {'name': 'Trang điểm mắt'},
        {'name': 'Mỹ phẩm nam'},
        {'name': 'Tắm & Chăm sóc cơ thể'},
        {'name': 'Chăm sóc tóc'},
        {'name': 'Chăm sóc răng miệng'},
        {'name': 'Máy massage & Thiết bị y tế'},
        {'name': 'Dụng cụ làm ddepj'},
        {'name': 'Vệ sinh phụ nữ & Hỗ trợ tình dục'},
        {'name': 'Nước hoa'},
        {'name': 'Vitamin & Thức phẩm chức năng'},
        {'name': 'Hóa Mỹ Phẩm & Khác'}
      ].toList()
    },
    {
      '8': [
        {'name': 'Máy ảnh du lịch & chụp lấy liền'},
        {'name': 'Thẻ Nhớ'},
        {'name': 'Camera giám sat & Webcam'},
        {'name': 'Máy ảnh DSLR'},
        {'name': 'máy ảnh không gương lật'},
        {'name': 'Máy quay phim'},
        {'name': 'phụ kiện máy ảnh'}
      ].toList()
    },
    {
      '9': [
        {'name': 'Giày cao gót'},
        {'name': 'giầy đế bằng'},
        {'name': 'Sandal'},
        {'name': 'Giày cao gót/Giày đế xuồng'},
        {'name': 'Bốt'},
        {'name': 'Guốc/Dép nữ'},
        {'name': 'Giày sneaker và Giày thể thao'},
        {'name': 'Phụ kiện giày'}
      ].toList()
    },
    {
      '10': [
        {'name': 'Đồng hồ nam'},
        {'name': 'Đồng hồ nữ'},
        {'name': 'Đồng hồ trẻ em'},
        {'name': 'Phụ kiện đồng hồ'}
      ].toList()
    },
    {
      '11': [
        {'name': 'Túi đeo chéo nữ'},
        {'name': 'Túi xách nữ'},
        {'name': 'Balo thời trang'},
        {'name': 'Ví/bóp nữ'},
        {'name': 'Cặp văn phòng'},
        {'name': 'Túi đựng tiện ích'},
        {'name': 'Túi vải/Túi tote'}
      ].toList()
    },
    {
      '12': [
        {'name': 'Giày thể thao/Sneakers'},
        {'name': 'Giày lười'},
        {'name': 'Giày tây'},
        {'name': 'Xăng-đan'},
        {'name': 'Dép'},
        {'name': 'Giày dép/Unisex'},
        {'name': 'Phụ kiện giàu dép'}
      ].toList()
    },
    {
      '13': [
        {'name': 'Trang sức'},
        {'name': 'kính mắt'},
        {'name': 'Nón & Dù'},
        {'name': 'Khăn,tất & Găng tay'},
        {'name': 'Phụ kiện tóc'},
        {'name': 'Dây lưng'},
        {'name': 'Hình xăm'},
        {'name': 'Phụ kiện cưới'},
        {'name': 'Khẩu trang'}
      ].toList()
    },
    {
      '14': [
        {'name': 'Đồ gia dụng nhà bếp'},
        {'name': 'Quạt & Máy nóng lạnh'},
        {'name': 'Thiết bị chăm sóc quần áo'},
        {'name': 'Máy hút bụi & THiết bị làm sạch'},
        {'name': 'Đồ gia dụng lớn'},
        {'name': 'khác'}
      ].toList()
    },
  ];
}
